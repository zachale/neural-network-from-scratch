...
class layer:
    def __init__(self, incoming, weights) -> None:
        i = 0
        self.nodes=[]
        while i < x:
            neuron = node(incoming, weights)
            self.nodes
...
def network (x,y, w, b):
    ...
    n21 = node([n1.value, n11.value], [w[1], w[2]])
    n21.bias = b[1]
    n22 = node([n1.value, n11.value], [w[3], w[4]])
    n22.bias = b[2]
    n23 = node([n1.value, n11.value], [w[5], w[6]])
    n23.bias = b[3]
    ...
    n31 = node([n21.value, n22.value, n23.value], [w[7],w[8],w[9]])
